...
void scx_bpf_error_bstr(char *fmt, unsigned long long *data,
			__u32 data_len) __ksym;
void scx_bpf_dump_bstr(char *fmt, unsigned long long *data,
		       __u32 data_len) __ksym __weak;
__u32 scx_bpf_cpuperf_cap(s32 cpu) __ksym __weak;
__u32 scx_bpf_cpuperf_cur(s32 cpu) __ksym __weak;
void scx_bpf_cpuperf_set(s32 cpu, __u32 perf) __ksym __weak;
...
#define scx_bpf_exit(code, fmt, args...)                            \
	({                                                          \
		scx_bpf_bstr_preamble(fmt, args) scx_bpf_exit_bstr( \
			code, ___fmt, ___param, sizeof(___param));  \
...
#define scx_bpf_error(fmt, args...)                                  \
	({                                                           \
		scx_bpf_bstr_preamble(fmt, args) scx_bpf_error_bstr( \
			___fmt, ___param, sizeof(___param));         \
		___scx_bpf_bstr_format_checker(fmt, ##args);         \
	})
...
#define MEMBER_VPTR(base, member)                                              \
	(typeof((base)member) *)({                                             \
		...
		_Static_assert(                                                \
			sizeof(base) >= sizeof((base)member),                  \
 ...
 */
static inline __u32 log2___u32(__u32 v)
{
	__u32 r;
	__u32 shift;
 ...
 * @v: The value for which we're computing the base 2 logarithm.
 */
static inline __u32 log2_u64(u64 v)
{
	__u32 hi = v >> 32;
	if (hi)
